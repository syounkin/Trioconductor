...
o <- order(DF$p.vec)
pdf(file = "./figures/cifigcleftnew-by-pvalue.pdf", width = 7, height = 12)

# o <- 1:length(DF$p.vec)
# pdf(file = "./figures/cifigcleftnew-by-pos.pdf", width = 7, height = 12)

par(mai=c(1,3,1,0.25), bg = "lightyellow")
#layout(matrix(1:2,nrow=1,ncol=2))
plot(1, type = "n", ylim = c(1,nrow(ci.mat.cleft)), xlim = c(0,1), ylab = "", xlab = "Cleft Transmission Rate", main = "", axes = FALSE )
...
for( i in 1:nrow(ci.mat.cleft) ){
     ...
     lines( x = c(ci.mat.cleft[o,1][i],ci.mat.cleft[o,2][i]), y = rep(nrow(ci.mat.cleft)-i+1,2), lwd = 3, col = rgb(1,0,0,alpha))
...
for( i in 1:nrow(ci.mat.con) ){
     ...
     lines( x = c(ci.mat.con[o,1][i],ci.mat.con[o,2][i]), y = rep(nrow(ci.mat.con)-i+1,2), lwd = 3, col = rgb(0,0,1,alpha))
...
axis(2, at = at <- (max(x) - 0:(max(x)-1)), labels = paste0(seqnames(DF$grange[o]),":",start(DF$grange[o]),"-",end(DF$grange[o])), las = 2 )
axis(3, at = at <- c(0.5,1), labels = round(c(0,max(-log10(DF$p.vec))),1))

mtext("-log10(p)",3, at = 0.75 )
...
points(0.5+-log10(DF$p.vec[o])/2/max(-log10(DF$p.vec[o])), y = length(o):1, pch = 20, col = "black", cex = 1 )
# lines(-log10(DF$p.vec[o])/max(-log10(DF$p.vec[o])), y = length(o):1, lty = 3 )
...
plot(DF$dist.vec.order.dist.vec.., DF$transrate.con.testable., pch = 20, col = ifelse(DF$dist.vec.order.dist.vec..<=1e6,"red","black"))
...
<<chr7, fig = TRUE, echo = FALSE>>=
chr <- 7
wd <- 10e3
chr7.region <- DF$grange[22]
...
plotTracks(list(dtrack,  gtrack, itrack ),  background.panel = "#FFFEDB", background.title = "darkblue", from = start(chr7.region)-wd, to = end(chr7.region)+wd )
@

=== Chromosome 8 ===

<<chr8, fig = TRUE, echo = FALSE>>=
chr <- 8
wd <- 10e3
chr8.region <- DF$grange[28]
...
itrack <- IdeogramTrack(genome = "hg18", chromosome = paste0("chr", chr), lty = 1, lwd = 1 )
plotTracks(list(dtrack,  gtrack, itrack ),  background.panel = "#FFFEDB", background.title = "darkblue", from = start(chr8.region)-wd, to = end(chr8.region)+wd )
@

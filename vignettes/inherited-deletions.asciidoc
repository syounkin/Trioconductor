...
<<cnvmatrix, echo = TRUE, eval = FALSE, cache = TRUE>>=
...
Five distinct but nearby regions on chromosome 6 appear between 29,940,311 and 32,788,048 (hg18) with transmission rates of 0.540, 0.483, 0.444, 0.430, 0.245.  The exceptionally low transmission rate for the last of these regions raised eyebrows and upon inspection of the genomic location we immediately observed that 4 of 5 of these regions contain a gene in the HLA family.  The HLA family of genes are known to be...

.Chromosome 6 Regions & HLA
[width="60%",cols="3"]
[options="header",grid="rows"]
|======
|UCSC Genome Browser (hg18)|band
|link:http://genome.ucsc.edu/cgi-bin/hgTracks?db=hg18&omimGene=full&decipher=full&position=chr6:29940311-30032810[chr6:29940311-30032810]|6p21.33|0.540
|link:http://genome.ucsc.edu/cgi-bin/hgTracks?db=hg18&omimGene=full&decipher=full&position=chr6:31382534-31422222[chr6:31382534-31422222]|6p21.33|0.483
|link:http://genome.ucsc.edu/cgi-bin/hgTracks?db=hg18&omimGene=full&decipher=full&position=chr6:32055573-32124218[chr6:32055573-32124218]|6p21.32|0.444
|link:http://genome.ucsc.edu/cgi-bin/hgTracks?db=hg18&omimGene=full&decipher=full&position=chr6:32536895-32684456[chr6:32536895-32684456]|6p21.32|0.430
|link:http://genome.ucsc.edu/cgi-bin/hgTracks?db=hg18&omimGene=full&decipher=full&position=chr6:32694224-32788048[chr6:32694224-32788048]|6p21.32|0.245
|======
...
<<filtercmp>>=
dist.vec <- as.data.frame(distanceToNearest(DF.cmp$grange,filter.gr))$distance
#DF.cmp <- DataFrame(DF.cmp[order(dist.vec),],dist.vec[order(dist.vec)])
DF.cmp <- DataFrame(DF.cmp, dist.vec)
@


<<disthist, fig = TRUE, echo = FALSE, height = 4, width = 4>>=
...
<<disthistpdf, echo = FALSE, results = hide>>=
...
<<disttrans, fig = TRUE, width = 12, height = 6, echo = FALSE>>=
...
<<filterout, fig = TRUE, height = 6, width = 6, echo = FALSE>>=
...
<<filteroutpdf, echo = FALSE, results = hide>>=
...
<<chr7, fig = TRUE, echo = FALSE, cache = FALSE, width = 12, height = 6, eval = FALSE>>=
chr <- 7
wd <- 100e3
chr7.region <- DF$grange[25]
chr7.index <- subjectHits(findOverlaps(chr7.region,gr.cmp))
...
dtrack <- DataTrack( range = gr.cmp[chr7.index], data = t(TU.mat.cmp[chr7.index,]), groups = colnames(TU.mat.cmp), type = "S", cex = 1, name = "Count", ylim = c(0,200), legend = TRUE, col = c(rep("red",2),rep("blue",2)), lty = 1:2, lwd = 2 )
...
<<chr7pdf, echo = FALSE, cache = FALSE, results = hide, eval = FALSE>>=
...
<<chr8, fig = TRUE, echo = FALSE, cache = FALSE, width = 12, height = 6, eval = FALSE>>=
chr <- 8
chr8.region <- DF$grange[26]
chr8.index <- subjectHits(findOverlaps(chr8.region,gr.cmp))
chr8.index2 <- subjectHits(findOverlaps(chr8.region,DF.cmp$grange))
gtrack <- GenomeAxisTrack()
grtrack <- GeneRegionTrack(txdb, genome="hg18", chromosome=chr, name="Entrez Gene ID")
dtrack <- DataTrack( range = gr.cmp[chr8.index], data = t(TU.mat.cmp[chr8.index,]), groups = colnames(TU.mat.cmp), type = "S", cex = 1, name = "Count", ylim = c(0,200), legend = TRUE, col = c(rep("red",2),rep("blue",2)), lty = 1:2, lwd = 2 )
...
<<chr8pdf, echo = FALSE, cache = FALSE, results = hide, eval = FALSE>>=
...
<<cifigcleftnew, fig = TRUE, cache = FALSE, echo = FALSE, results = hide, height = 8, width = 10>>=


layout(matrix(1:2,nrow=1,ncol=2))
par(mai=c(1,2.5,0,0), bg = "lightyellow")

o <- order(DF$p.vec)
# o <- 1:nrow(ci.mat.cleft)

plot(1, type = "n", ylim = c(1,nrow(ci.mat.cleft)), xlim = c(0,1), ylab = "", xlab = "Transmission Rate", main = "", axes = FALSE )
...
par(mai=c(1,0,0,0), bg = "lightyellow")
plot(-log10(DF$p.vec[o]), y = length(o):1, pch = 20, col = "black", cex = 1.5, axes = FALSE, main = "", xlab = "-log10(p)", ylab = "", xlim = c(0,4))
lines(rep(-log10(0.05/sum(DF$dist.vec > 1e6)),2), c(1,length(o)), lty = 3)
lines(rep(-log10(0.05),2), c(1,length(o)), lty = 3)
axis(1)
@

<<cifigcleftnewpdf, cache = FALSE, echo = FALSE, results = hide>>=
pdf(file = "./figures/cifigcleftnew.pdf", width = 12, height = 8)
par(bg="lightyellow")
...
<<qqregion, fig = TRUE, width = 6, height = 6, eval = FALSE, echo = FALSE>>=
...
<<qqPlotregion, fig = TRUE, width = 6, height = 6>>=
library("snpStats")
par(bg="lightyellow")
...
<<qqPlotcmp, fig = TRUE, width = 6, height = 6>>=
par(bg="lightyellow")

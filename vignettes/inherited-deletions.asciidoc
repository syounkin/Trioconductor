...
<<options, echo=TRUE, eval = TRUE >>=
  ...
  data(cleansamples, package="CleftCNVAssoc")
...
<<pmin, eval = TRUE, echo = FALSE, cache = FALSE>>=
...
@ 
<<cumsum, fig = TRUE, width = 8, eval = FALSE, echo = FALSE, cache = FALSE>>=
...
lines( c(0,xlim[2]), c(0,xlim[2]), lty = 3 )
@ 
<<cumsum2, fig = TRUE, width = 8, eval = FALSE, echo = FALSE, cache = FALSE>>=
...
<<trioStates3, eval = TRUE, echo = FALSE, results = verbatim, cache = FALSE>>=
trans.mat.clp <- c()
...
<<chr7, fig = TRUE, echo = FALSE, cache = FALSE, width = 12, height = 6, eval = TRUE>>=
chr <- 7
wd <- 100e3
...
<<chr7pdf, echo = FALSE, cache = FALSE, results = hide, eval = TRUE>>=
...
<<chr8, fig = TRUE, echo = FALSE, cache = FALSE, width = 12, height = 6, eval = TRUE>>=
chr <- 8
...
<<chr8pdf, echo = FALSE, cache = FALSE, results = hide, eval = TRUE>>=
...
plot(-log10(DF$p.vec[o]), y = length(o):1, pch = 20, col = "black" , axes = FALSE, main = "", xlab = "-log10(p)", ylab = "", xlim = c(0,4), ylim = c(1,length(o)+1) )
...
cD <- DataFrame(colData(fe.beaty)$id)
colnames(cD) <- "id"
colData(fe.beaty) <- cD
...
@
And the threshold for niminal p-values is:
<<padj, eval = FALSE>>=

...
<<pmin, echo = TRUE, eval = TRUE>>=
regions.gr <- reduce(DF$grange)
index.vec <- subjectHits(findOverlaps(DF$grange,regions.gr))
DF.list <- split(DF, index.vec)
p.min.vec <- as(unlist(lapply( DF.list, function( obj ) min(obj$p.vec, na.rm = TRUE) )),"numeric")
n.vec <- as(unlist(lapply( DF.list, function( obj ) nrow(obj))),"numeric")
p.median.vec <- as(unlist(lapply( DF.list, function( obj ) median(obj$p.vec, na.rm = TRUE))),"numeric")
trans.median.vec <- as(unlist(lapply( DF.list, function( obj ) median(obj$trans.vec, na.rm = TRUE))),"numeric")
@ 
<<meta, eval = TRUE, echo = FALSE>>=
meta <- values(regions.gr)
meta <- DataFrame(meta, p.min = p.min.vec, p.median = p.median.vec, trans.median = trans.median.vec, n.cmp = n.vec)

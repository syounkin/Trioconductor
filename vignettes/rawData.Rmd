...
```{r logrplot, fig.width = 24, echo = FALSE}
par(bg="lightyellow")
...
for( i in which(names(logr.list) %in% offspring.chr16) ){
  ...
  for( j in 1:length(foo) ) lines(c(start(foo[j]),end(foo[j])),rep(2,2))
  }
```
...
head(offspring.chr16)
```
```{r logrplot3, fig.width = 24, echo = FALSE}
par(bg="lightyellow")
for( i in which(names(logr.list) %in% offspring.chr16) ){
  plot(1, type = 'n', xlim = range(pos.vec), ylim = c(-2,6) )
  points(pos.vec, logr.list[[i]], pch = 20, col = "orange")

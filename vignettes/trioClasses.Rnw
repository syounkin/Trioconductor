...
(scan.trio <- ScanTrio( object = ste.rare, window = rowData(ste.rare[1:10]) + 1e3/2 , block = range(rowData(ste))))
@ 
...
\subsection{Window}
<<windows>>=
window <- GRanges( seqnames =  "chr14", IRanges(start(gr[100]), width = 10e3), strand = "*")
window2 <- GRanges( seqnames = "chr14", IRanges(start(gr[100]), width = 1e3), strand = "*")
@ 
<<transcount>>=
#window

...
@ 
\section{Methods}
Now create a list of three SnpMatrices for trio analyses using GenoTrio().
<<genotrio>>=
( gtrio <- GenoTrio(ste) )
@ 
Now we can do things easily, such as count transmissions.
<<trans>>=
with( gtrio, sum( F == 1 & M == 2 & O == 2, na.rm = TRUE) )
@ 
Or run Holger's functions.
<<trio>>=
geno <- ctcbind(gtrio)
aTDT(geno)$transMinor[1:10]
@ 
I haven't confirmed yet that TransCount is working properly.  The counts seem high, but I'm not sure if the comparision is correct.
<<transcount>>=
TransCount(ste,gr[1])
TransCount(ste,gr[2])
TransCount(ste,gr[3])

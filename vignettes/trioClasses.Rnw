...
@ 
<<snpEx>>=
snpEx <- new("SNPTrioExperiment", Pedigree = data.frame(1:10), assays = SimpleList(geno = sm[1:13,1:10], logR = logR, baf = baf), colData = df, rowData = targets.gr )
show(snpEx)
class(assays(snpEx)[[1]])
@ 
\end{document}

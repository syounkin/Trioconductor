...
#data("ped")
#ped.df <- subset( ped.df, !is.na(id) )
#coldata.df <- subset(ped.df, id %in% rownames(sm))
#ped.DF <- with(ped.df, {
#  DataFrame(
#    famid = ped,
#    id = id,
...
#    )
#})
#ped.DF <- as(ped.df, "DataFrame")
#rownames(sm) <- paste0("subject", 1:nrow(sm))
#ped.DF <- DataFrame( famid = NA, id = rownames(sm), fid = NA, mid = NA, sex = NA, dx = NA ) 
#col.DF <- as( coldata.df, "DataFrame" )
#rownames(col.DF) <- col.DF$id
#gr <- GRanges( seqnames = "chr14", ranges = IRanges(start = 1:ncol(sm), width = 1) )
...
colData(ste)
@ 
Now create a list of three SnpMatrices for trio analyses.
<<genotrio>>=
GenoTrio(ste)
@ 
\section{Validity}

...
@ 
<<se>>=
gr <- GRanges( seqnames = paste0("snv",1:nrow(sm)), ranges = IRanges(start = 1:nrow(sm), width = 1) )
df <- DataFrame( dx = rep(1,ncol(sm)), sex = rep(1,ncol(sm)), row.names = colnames(sm) )
se <- SummarizedExperiment(assays = SimpleList(geno = sm), colData = df, rowData = gr )
@ 
<<ste>>=

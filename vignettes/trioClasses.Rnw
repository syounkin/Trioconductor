...
aTDT(ste[1:5])
@ 
\subsection{Count of Transmission of Rare Variants (\emph{TransCount()})}
First we define the world and a block.
<<block>>=
world <- range(gr)
block <- GRanges( seqnames = "chr14", IRanges(start(gr[100]), width = 10e3), strand = "*")
@ 
\subsubsection{In a block}
<<transcount>>=
block
TransCount( ste[MAF(ste) <= 0.01,], block )
@ 
\subsubsection{Not In the block}
<<transcount>>=
setdiff(world,block)
TransCount( ste[MAF(ste) <= 0.01,], setdiff(world,block) )
@ 
\subsubsection{In the ``world''}
<<transcount>>=
world
TransCount( ste[MAF(ste) <= 0.01,], world )
@ 
\subsection{Iteratively}
<<iter>>=
grlist <- GRangesList(gr[1],gr[2])
trans.vec <- TransCount( ste, grlist )
#trans.vec <- TransCount( ste[MAF(ste) <= 0.01,], grlist )
@ 
<<transvec>>=
trans.vec
@ 

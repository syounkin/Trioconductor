...
<<window-show>>=
window
range(rowData(ste.rare))
countOverlaps(window, rowData(ste.rare))
@ 
%Create a GRanges object such that each range contain \Sexpr{n.snp} rare variants.
<<scantrio>>=
#window <- rowData(ste.rare) + 250e3
...
\end{figure}
<<save>>=
save(scan.trio, file = "./../data/scan-trio.RData")
@ 
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\clearpage
\appendix
\section{Holger-style Genotype Matrix}
Coercion to matrix for \emph{trio}
<<trio, eval = FALSE>>=
geno <- as(ste, "matrix")
aTDT(geno[,1:5])
@ 
Or apply the aTDT method to the FamilyExperiment directly.
<<trio2, eval = FALSE>>=
aTDT(ste[1:5])
@ 
\clearpage
...
Now, we include the pedigree information as an object of class PedClass.  We keep PedClass independent of FamilyExperiment for flexibility.
<<ste, eval = FALSE>>=
ped <- PedClass(ped.DF)
ste <- FamilyExperiment(se, pedigree = ped )
@ 
...
getClass("FamilyExperiment")
@ 
\subsection{PedClass}
Now we investigate the pedigree slot of the FamilyExperiment object.
<<classes, eval = FALSE>>=
class(pedigree(ste))
getClass("PedClass")
pedigree(ste)

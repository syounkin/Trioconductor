...
gr.deletion <- gr.beaty[values(gr.beaty)$numsnp >= 10 & values(gr.beaty)$cn %in% 0:1 ]
#data("sample")
...
@
<<cnvmatrix>>=
#system.time( cnv <- CNVMatrix( gr.deletion ) )
#system.time( cnv.mat <- CNVMatrix( gr.beaty[1:4e3] ) )
system.time( cnv.mat <- CNVMatrix( gr.beaty[1:1e2] ) )
@ 
<<savecnv>>=
save( cnv, file = "./../data/cnv.RData" )
@ 
\end{document}

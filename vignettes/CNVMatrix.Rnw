...
@ 
<<cnvmatrix-beaty>>=
system.time( cnv.obj.beaty <- CNVMatrix( gr.deletion.beaty.clean, gr = gr.deletion.pitt.clean[1:10] ) )
@ 
<<cnvmatrix-pitt, eval = FALSE, echo = FALSE>>=
system.time( cnv.obj.pitt <- CNVMatrix( gr.deletion.pitt.clean, gr = gr.deletion.beaty.clean ) )
@ 
<<savecnv, echo = FALSE, eval = FALSE>>=

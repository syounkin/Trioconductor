...
``` 
```{r trioStates, eval = TRUE, echo = FALSE, cache = TRUE}
...
``` 
```{r table-list, eval = TRUE, echo = FALSE, cache = TRUE}
...
```{r pmin, eval = TRUE, echo = FALSE, cache = TRUE}
...
```{r cumsum, fig = TRUE, width = 8, eval = TRUE, echo = FALSE, cache = TRUE}
...
```{r cumsum2, fig = TRUE, width = 8, eval = TRUE, echo = FALSE, cache = TRUE}
#TranscriptDb <- TxDb.Hsapiens.UCSC.hg18.knownGene
#chrlength <- c(0,seqlengths(TranscriptDb)[paste0("chr",1:22)])
#names(chrlength) <- c(names(chrlength[-1]),"foo")
#chrlength <- rev(rev(chrlength)[-1])
...
pos.vec <- getPosition(beaty_snpAnnot)
chr.vec <- getChromosome(beaty_snpAnnot)
diff.vec <- diff(pos.vec)
pos.vec <- rev(rev(pos.vec)[-1])
chr.vec <- rev(rev(chr.vec)[-1])
indx <- chr.vec==16 & pos.vec >= (32404517-1e6) & pos.vec <= (32530051+1e6)
plot(spline(pos.vec[indx],1/diff.vec[indx], n = ceiling(diff(range(pos.vec[indx]))/75e3)), type = "l", ylim = c(0,0.05))
indx <- chr.vec==15 & pos.vec >= (19768826-1e6) & pos.vec <= (19982036+1e6)
plot(spline(pos.vec[indx],1/diff.vec[indx], n = ceiling(diff(range(pos.vec[indx]))/75e3)), type = "l", ylim = c(0,0.05))
indx <- chr.vec==7 & pos.vec >= (141419097-1e6) & pos.vec <= (141441259+1e6)
plot(spline(pos.vec[indx],1/diff.vec[indx], n = ceiling(diff(range(pos.vec[indx]))/75e3)), type = "l", ylim = c(0,0.05))
indx <- chr.vec==8 & pos.vec >= (39356825-1e6) & pos.vec <= (39497557+1e6)
plot(spline(pos.vec[indx],1/diff.vec[indx], n = ceiling(diff(range(pos.vec[indx]))/75e3)), type = "l", ylim = c(0,0.05))
```
...
```{r logrplot-chr6, fig.width = 24, echo = FALSE, cache = FALSE, eval = FALSE}
par(bg="lightyellow")
...
```{r logrplot2-chr6, fig.width = 24, fig.height = 10, echo = FALSE, cache = FALSE}
par(bg="lightyellow")
...
head(offspring.chr6)
```
```{r logrplot3-chr6, fig.width = 24, fig.height = 10, echo = FALSE, cache = FALSE}
par(bg="lightyellow")
for( i in which(names(logr.list) %in% offspring.chr6) ){
  plot(1, type = 'n', xlim = range(pos.vec), ylim = c(-1,11), axes = FALSE, main = paste0(names(logr.list)[i]), xlab = "MB", ylab = "")

...
gr.cnp.beaty <- cnv.beaty.obj$cmp.gr[testable.beaty]
@ 
...
gr.cnp.pitt <- cnv.pitt.obj$cmp.gr[testable.pitt]
@ 
...
\begin{itemize}
  ...
  \item coverage $>10$
  \item \Sexpr{length(cnv.beaty.obj$gr)} hemi/homozygous deletions identified in \Sexpr{nrow(completeTrios(fe.beaty))} trios
...
freq.vec.beaty.chr <- freq.vec[as.logical(seqnames(gr.cnp.beaty) == paste0("chr", chr))]
est.vec.pitt.chr <- est.vec.pitt[as.logical(seqnames(gr.cnp.pitt) == paste0("chr", chr))]
freq.vec.pitt.chr <- freq.vec[as.logical(seqnames(gr.cnp.pitt) == paste0("chr", chr))]
...
dtrack <- DataTrack( range = gr.cnp.beaty.chr, data = est.vec.beaty.chr, type = "b", cex = 1, name = "Trans (Clefts)", ylim = c(0,1), col = "darkblue", pch = 20 )
...
<<transmission-rate>>=
mean(est.vec.beaty[subjectHits(findOverlaps(reduce(locus), cnv.beaty.obj$cmp.gr))])
mean(est.vec.pitt[subjectHits(findOverlaps(reduce(locus), cnv.pitt.obj$cmp.gr))])
@  

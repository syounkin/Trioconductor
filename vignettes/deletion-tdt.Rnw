...
(se <- SummarizedExperiment(assays = SimpleList(cnv = t(cnv$cnv.mat)), colData = DataFrame(id=rownames(cnv$cnv.mat), row.names = rownames(cnv$cnv.mat)), rowData = cnv$cmp.gr ))
@ 
\section{Pedigree}
<<beaty-ped>>=
...
reduce(cnv$cmp.gr[which(trans.vec <= 0.05/length(trans.vec))])
@ 
\end{document}

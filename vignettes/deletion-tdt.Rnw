...
dtrack <- DataTrack( range = gr.cnp.chr, data = -log10(p.vec.chr), type = "b", cex = 1, name = "-log10(p)", ylim = c(0,10) )
dtrack2 <- DataTrack( range = gr.cnp.chr, data = freq.vec.chr, type = "b", col = "red", cex = 1, name = "freq", ylim = c(0,1) )
...
fud <- 1e5/2
plotTracks(list(dtrack, dtrack2, gtrack, itrack ),  background.panel = "#FFFEDB", background.title = "darkblue", from = 39356825 - fud, to = 39497557 + fud)
@ 
\end{center}

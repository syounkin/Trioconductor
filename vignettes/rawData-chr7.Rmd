...
```{r grange}
gr <- GRanges( seqnames = rep('chr7',length(offspring.vec)), ranges = IRanges( start = 141419097, end = 141441259), id = offspring.vec )
```
...
```{r logrplot-chr7, fig.width = 24, echo = FALSE, cache = FALSE}
par(bg="lightyellow")
...
trioStates.chr7 <- with(trioAssay.chr7, matrix( paste0(F,M,O), nrow = nrow(O), ncol = ncol(O)))
...
```{r untrans}
untrans.mat <- matrix(trioStates.chr7 %in% c("100","010","110"),nrow=nrow(trioStates.chr7),ncol=ncol(trioStates.chr7),byrow=FALSE,dimnames=dimnames(trioStates.chr7))
...
```{r logrplot2-chr7, fig.width = 24, echo = FALSE, cache = FALSE}
par(bg="lightyellow")
for( i in which(names(logr.list) %in% offspring.chr7) ){
  ...
  foo <- reduce(rowData(fe.beaty.chr7)[untrans.mat[names(logr.list)[i],]])
...
```{r trans}
trans.mat <- matrix(trioStates.chr7 %in% c("101","011","112"),nrow=nrow(trioStates.chr7),ncol=ncol(trioStates.chr7),byrow=FALSE,dimnames=dimnames(trioStates.chr7))
```
```{r offspring2}
...
head(offspring.chr7)
```
```{r logrplot3-chr7, fig.width = 24, echo = FALSE, cache = FALSE}
par(bg="lightyellow")
for( i in which(names(logr.list) %in% offspring.chr7) ){
  plot(1, type = 'n', xlim = range(pos.vec), ylim = c(-2,6) )
  points(pos.vec, logr.list[[i]], pch = 20, col = "purple")
  points(pos.vec, 4+logr.fa.list[[i]], pch = 20, col = "red")
  points(pos.vec, 4+logr.ma.list[[i]], pch = 20, col = "blue")
  foo <- reduce(rowData(fe.beaty.chr7)[trans.mat[names(logr.list)[i],]])

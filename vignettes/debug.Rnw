...
<<small>>=
ste.small <- ste[1:50]
bad.markers <- which(colSums(is.na(geno(ste.small)))>0)
ste.small <- ste.small[-bad.markers]
wd <- 100
window <- GRanges( seqnames = seqnames(ste.small), ranges = IRanges(start = start(rowData(ste.small)), width = wd ) )
